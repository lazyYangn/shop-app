    ...
    <my-content>
      ...
      >
        ...
        <a-form-model-item style="margin-top: 16px">
          <a-button @click="sub('loginForm')" size="large" type="primary" block
            >登录</a-button
          >
        </a-form-model-item>
      </a-form-model>
...
import { setCachVal } from "@/kits/LocalStorage";
export default {
  name: "Login",
  data() {
    return {
        ...
        if (valid) {
          ...
          try {
            if (res.code == 1) {
              ...
              setCachVal("userid", res.data.userId);
              setCachVal("username", res.data.userName);
              console.log(res.msg);
