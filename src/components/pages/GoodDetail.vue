...
export default {
  ...
  created() {
    ...
    if (getCachVal("token") && getCachVal("token").length > 0) {
      this.visitedGood(
        getCachVal("userid"),
        this.$store.state.selectGoods[this.$store.state.selectGoods.length - 1]
          .id
      );
    }
  },
      ...
      if (getCachVal("token") && getCachVal("token").length > 0) {
        this.visitedGood(
          ...
          this.$store.state.selectGoods[
            this.$store.state.selectGoods.length - 1
          ].id
        );
      }
    },
  ...
  methods: {
    ...
    async initData() {
      const id =
        this.$store.state.selectGoods &&
        this.$store.state.selectGoods.length > 0
          ? this.$store.state.selectGoods[
              this.$store.state.selectGoods.length - 1
            ].id
          : "";
